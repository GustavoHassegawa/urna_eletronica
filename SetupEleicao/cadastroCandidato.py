...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.comboPartidoCandidato.activated.connect(self.alterarFotoPartido)
    ...
    def alterarFotoPartido(self, text):
        foto = database.getFotoPartido(self.comboPartidoCandidato.itemText(text))
    ...
    def btnCadastrarClicked(self):
        if self.txtNomeCandidato.text() == "":
            ...
            notify2.init(u"Urna Eletrônica")
            notificacao = notify2.Notification(u'Oops', u'Você esqueceu de inserir o título')
            notificacao.show()
        elif self.comboCargoCandidato.currentText() == "":
            notify2.init(u"Urna Eletrônica")
            notificacao = notify2.Notification(u'Oops', u'Você esqueceu de selecionar um cargo')
            notificacao.show()
        elif self.comboPartidoCandidato.currentText() == "":
            notify2.init(u"Urna Eletrônica")
            notificacao = notify2.Notification(u'Oops', u'Você esqueceu de selecionar um partido')
            notificacao.show()
            ...
            notify2.init(u"Urna Eletrônica")
            notificacao = notify2.Notification(u"OK", u"Candidato cadastrado com sucesso.")
            notificacao.show()
            pixmap = QPixmap(ICON)
            self.lblFoto.setPixmap(pixmap)
            self.lblFotoName.setText(ICON)
